// htmlж ‡з­ѕеўћеЉ з‰№е®љclass, attributeпјЊй™ђе®љж ·ејЏпјЊе…је®№жµЏи§€е™Ёз”Ё
...
huawei.menu_click = function () {
	// èœå•åˆ‡æ¢éšè—å…¶ä»–å·²å±•å¼€çš„èœå•
...
huawei.BrowseHappy = function BrowseHappy() {
	...
	//åˆ é™¤ä¸­æ–‡ç«™ç‚¹ä¸‹èŽ·å–ä½¿ç”¨cookieæç¤º
...
huawei.throttle = function _throttle(fn, time) {
	...
	var _self = fn,
	timer = void 0,
	firstTime = true; //è®°å½•æ˜¯å¦æ˜¯ç¬¬ä¸€æ¬¡æ‰§è¡Œçš„flag

	return function () {
		var args = arguments,
		//è§£å†³é—­åŒ…ä¼ å‚é—®é¢˜
		_me = this; //è§£å†³ä¸Šä¸‹æ–‡ä¸¢å¤±é—®é¢˜

		if (firstTime) {
			//è‹¥æ˜¯ç¬¬ä¸€æ¬¡ï¼Œåˆ™ç›´æŽ¥æ‰§è¡Œ
			_self.apply(_me, args);
			return firstTime = false;
		}
		if (timer) {
			//å®šæ—¶å™¨å­˜åœ¨ï¼Œè¯´æ˜Žæœ‰äº‹ä»¶ç›‘å¬å™¨åœ¨æ‰§è¡Œï¼Œç›´æŽ¥è¿”å›ž
			return false;
		}
...
huawei.lazyload = function (selector) {
	...
	$imgs.lazyload({
		...
		load: setWH //åœ¨imgè§¦å‘loadäº‹ä»¶æ—¶æ‰§è¡Œçš„å›žè°ƒ
	});
...
huawei.lazyload_init = function (selector) {
	var dtd = $.Deferred(); // æ–°å»ºä¸€ä¸ªdeferredå¯¹è±¡
	...
	$('img.lazy').each(function (i, o) {
		...
		try {
			$t.parent().setWidthHeight({
				'scale': parseInt($t.attr("width")) / parseInt($t.attr("height")) // ä¼ å›¾ç‰‡æ¯”ä¾‹
			})
			.end().removeAttr("width").removeAttr("height");
		} catch (_e) { console.log(_e); }
	});

	dtd.resolve(); // æ”¹å˜deferredå¯¹è±¡çš„æ‰§è¡ŒçŠ¶æ€
...
$.fn.extend({
	'setWidthHeight': function (options) {
		...
		//ä½¿ç”¨jQuery.extentè¦†ç›–æ’ä»¶é»˜è®¤å‚æ•°
...
huawei.text_dot = function () {
	...
	$(".js-text-dot-cn").dotdotdot({
		wrap: 'letter', //æ³¨ï¼šä¸­æ–‡å¿…é¡»æ”¹ä¸ºletter
		ellipsis: "..."
	});
};
...
function GetUserNameInfo() {
    try {
        // ç™»é™†æ³¨é”€é“¾æŽ¥åè®®åˆ¤æ–­ by zwx5316375
    ...
    var IsBackLogin = $.cookie("login_uid");//åŽå°ç™»å½•
